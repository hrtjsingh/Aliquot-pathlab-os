...
export default async function DashboardPage() {
  ...
  const criticalOpen = await prisma.result.count({
    where: { flag: { in: ["CRITICAL_LOW", "CRITICAL_HIGH"] }, status: { not: "RELEASED" } },
  });
  const releasedToday = await prisma.order.count({
    where: { status: "RELEASED", reportedAt: { gte: new Date(new Date().setHours(0, 0, 0, 0)) } },
  });
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-6">
        ...
        hint={
          ...
          >
            {criticalOpen > 0
              ...
              : "Use Worklist for samples in progress. Register a patient first if you are creating a new accession."}
          </InstructionAlert>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
        {statuses.map((s, i) => (
          <Link key={s} href={"/worklist" as never} className="block">
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <p className="text-2xl font-semibold tabular">{releasedToday}</p>
          </CardContent>
        </Card>
      </div>
                ...
                <li key={o.id}>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="tabular text-xs text-muted-foreground">{o.accessionNo}</p>
                    </div>
